...
adminRouter.post('/admin/register',async(req,res) =>{
  try{
   ...
   let admin = await Admin.findOne({ userMail: req.body.userMail});
    if(admin){
      if(admin.isVerified){
      ...
      {
        let myquery = { userMail : admin.userMail};
...
adminRouter.post('/admin/login', function(req,res){
  ...
  var newvalues = { $set : {isLogged : true}};
      ...
      Admin.findOne({userMail : userMail}, function(err, admin){
        //console.log(admin);
      if(err){
        console.log("error");
      }
      ...
      if(admin){
        ...
        return res.send(JSON.stringify({"description":"Admin mail doesn't Exist... Create a new one!", "status":"failed"}));
      }
    }); 
  }
  catch(err){
...
adminRouter.post('/admin/logout',function(req,res){
  ...
  Admin.findOne({userMail : userMail}, function(err, admin){
    ...
    return res.status(404).send(JSON.stringify({"description":"You're not a registered admin...please register!", "status":"failed"}));
  }
});
}
catch(err){
...
try{
  ...
  var videos_id = req.body.videos;
    Channel.findOne({channelId : channel_id}, function(err, admin){
      if(admin){
...
adminRouter.post('/admin/removeUser',function(req,res){
  ...
  Admin.findOne({userMail:user_mail},function(err,admin){
    if(admin){
      Admin.deleteOne({userMail:user_mail},function(err,obj){
        if(err) throw err;
        console.log(user_mail + " deleted successfully");
        return res.status(200).send(JSON.stringify({"description":"Admin Info Deleted Successfully","status":"success"}));
      });
    }
    else{
...
try{
  ...
  Admin.findOne({firstName:"Vignesh"}, usersProjection, function (err, admin) {
    if (err) return next(err);
...
adminRouter.post('/admin/viewSessions',function(req,res){
  try{
    ...
    adminSession.find({userMail : req.body.userMail}, usersProjection).sort('-loginTime').exec(function (err, session) {
        if (err) return next(err);
